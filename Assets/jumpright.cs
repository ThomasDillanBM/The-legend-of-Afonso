    ...
    void OnCollisionEnter2D(Collision2D colisor)
    {
        ...
        if (colisor.gameObject.tag == "Player")
        {
            if (this.direction == RIGHT)
            {
                ...
                colisor.gameObject.GetComponent<Rigidbody2D>().AddForce((Vector2.right + Vector2.up) * this.force);
            }
            else
            {
                colisor.gameObject.GetComponent<Rigidbody2D>().Sleep();
                colisor.gameObject.GetComponent<Rigidbody2D>().WakeUp();
                colisor.gameObject.GetComponent<Rigidbody2D>().AddForce((Vector2.left + Vector2.up) * this.force);
            }
